...
[原仓库地址](https://github.com/271374667/NuitkaGUI)

## 我已修改：
已修复。问题原因：

1. UnicodeDecodeError : Windows 中文系统的命令行默认使用 GBK 编码 ，但代码使用了 encoding='utf-8' ，导致解码失败。
2. AttributeError : 当解码失败时， subprocess.run 的 stdout 返回 None ，调用 .splitlines() 就报错了。
修改内容：

- 将 encoding='utf-8' 改为 encoding='gbk'
- 添加 errors='replace' 参数，即使遇到无法解码的字符也不会崩溃
- 添加 None 检查，防止空值导致后续错误
